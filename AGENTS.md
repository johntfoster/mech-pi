...
- `Ctrl-a` then `c` creates a pmux Kitty tab, `n`/`p` switch next/previous pmux tabs, `1`...`9` jump directly to pmux tab numbers, and `x` closes the current pmux tab after confirmation.
